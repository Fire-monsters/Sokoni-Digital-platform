...
export function createDispatcherRouter(
  ...
  router.get("/delivery-riders", async (request, response, next) => {
    try {
  ...
  router.get("/deliveries/:deliveryId/nearby-riders", async (request, response, next) => {
  ...
  router.post("/deliveries/:deliveryId/assign", assign(false));
  router.post("/deliveries/:deliveryId/reassign", assign(true));

  router.post("/delivery-issues/:issueId/resolve", async (request, response, next) => {
  ...
  router.post("/deliveries/:deliveryId/actions", async (request, response, next) => {
    const params = riderDeliveryParamsSchema.safeParse(request.params);
  ...
  router.get("/deliveries/:deliveryId/evidence", async (request, response, next) => {
    const params = riderDeliveryParamsSchema.safeParse(request.params);
    if (!params.success) {
      sendZodValidationError(request, response, params.error.issues);
      return;
    ...
    try {
      sendSuccess(
        ...
        response,
        200,
        await proofService.getEvidence(
          request.auth.userId,
          request.auth.roles,
